...
## Practica ssh tres servidores


Configuracion servidor 2

Abajo en el fichero netplan, sin borrar nada, añadimos(usando el nombre de nuestro adaptador, tambien hay que usar una ip domestica ya que no nos funcionara con estas ips en virtualbox):
````
eth1:
    addresses:
        -10.20.0.20/16
...
Configuracion adaptador servidor 3
Y en este lo mismo
````
eth1:
    addresses:
...
Creamos las claves publica privada en el server a y la transeferimos al aquipo b y c y redireccionamos en el server b la clave publica al fichero authorized_keys
si no existe en este servidor el directorio .ssh lo creamos y hacemos lo mismo con el server c.
