...
Write-Output "Configuración actual: $($dnsServers[0]) - $($dnsServers[1])"


$dnsServers[1] = "8.8.8.8"

configurados
Write-Output "Total de servidores DNS configurados: $($dnsServers.Count)"


Write-Output "Configuración final: $($dnsServers[0]) - $($dnsServers[1])"
...
Write-Output "Rotación de backups: Del $viejo al $nuevo"
...
$incidencias.Add("Monitor parpadea") | Out-Null
$incidencias.Add("Ratón no va") | Out-Null

$incidencias.Insert(0, "SERVIDOR CAÍDO")

$incidencias.Remove("Ratón no va")

Write-Output "Lista actual de incidencias:"
$incidencias

Write-Output "Total de incidencias pendientes: $($incidencias.Count)"
...
$listaNegra.Add("10.10.10.5") | Out-Null
$listaNegra.Add("192.168.50.4") | Out-Null
$listaNegra.Add("80.80.80.80") | Out-Null
...
if ($listaNegra -contains $nuevaAmenaza) {
    Write-Output "La IP $nuevaAmenaza ya estaba bloqueada. No se hace nada."
} else {
    $listaNegra.Add($nuevaAmenaza) | Out-Null
    Write-Output "IP $nuevaAmenaza añadida a la lista negra."
}

Write-Output "Lista final de IPs bloqueadas:"
...
$puertos.Add(80)   | Out-Null
$puertos.Add(443)  | Out-Null
$puertos.Add(53)   | Out-Null


$puertos.Add(23)   | Out-Null

$puertos.Remove(23) | Out-Null


Write-Output "Puertos permitidos:"
$puertos | Sort-Object
...
$arrayOrigen = @("Spooler", "W3SVC", "LanmanWorkstation")


$servicios = [System.Collections.Generic.List[string]]::new($arrayOrigen)


$servicios.Add("wuauserv") | Out-Null

$serviciosOrdenados = $servicios | Sort-Object


foreach ($servicio in $serviciosOrdenados) {
    Write-Output "Monitorizando servicio: $servicio… OK"
}
...
# 2. Dividir la cadena usando el punto y coma ;
$partes = $logLine -split ";"


$userPart = $partes[0].Trim()          
$usuario = ($userPart -split ":")[1].Trim() 


$ipPart = $partes[1].Trim()           
$ip = ($ipPart -split ":")[1].Trim()  

Write-Output "ALERTA: El usuario $usuario intentó conectar desde $ip"
...
$cadenaCorreos = $correos -join ","


$resultadoCSV = "EmailAddress," + $cadenaCorreos

Write-Output $resultadoCSV
